...
const RELEASE_PRIORITY_WINDOW_DAYS = 30;
...
const isPriorityRelease = (item: Media, nowTime: number) => {
  if (item.media_type !== 'movie' && item.media_type !== 'game') return false;

  const date = getRelevantDate(item);
  if (!date || isNaN(date)) return false;

  const diffDays = Math.ceil((date - nowTime) / DAY_MS);
  return diffDays >= -RELEASE_PRIORITY_WINDOW_DAYS && diffDays <= RELEASE_PRIORITY_WINDOW_DAYS;
};
...
const sortByAddedWithReleasePriority = (list: Media[]) => {
  ...
  list.forEach((item) => {
    if (isUpcomingTv(item, nowTime) || isPriorityRelease(item, nowTime)) {
      upcoming.push(item);
    } else {
      added.push(item);
    }
